...
# 添加父目录到路径，以便导入angle_processor
sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
from processor.angle_processor import AngleProcessor

class SingleSequenceAngleProcessor:
    """单序列角度处理器，用于分析和可视化单个序列的后处理效果"""
    ...
    def process_single_sequence(self, sequence_id: int, frames_data: Dict[int, Dict]) -> Tuple[Dict[int, Dict], Dict[int, Dict], Dict]:
        ...
        # 使用angle_processor的process_sequence方法
    ...
    def create_visualization(self, original: Dict[int, Dict], 
        ...
        fig.suptitle(f'Sequence {sequence_id} Angle-based Post-processing Analysis', fontsize=16, fontweight='bold', color='white')
        ...
        output_path = f'single_sequence_results/sequence_{sequence_id}_angle_analysis.png'
        os.makedirs(os.path.dirname(output_path), exist_ok=True)
        plt.savefig(output_path, dpi=300, bbox_inches='tight', facecolor='black')
        print(f"角度分析可视化结果已保存到: {output_path}")
    ...
    def print_detailed_analysis(self, original: Dict[int, Dict], 
        ...
        print(f"\n{'='*60}")
        print(f"序列 {sequence_id} 角度分析详细报告")
        print(f"{'='*60}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='单序列角度后处理分析')
    ...
    # 创建单序列角度处理器
    processor = SingleSequenceAngleProcessor(
