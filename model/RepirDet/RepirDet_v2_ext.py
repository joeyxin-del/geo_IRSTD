...
class RepirDet(nn.Module):
    def __init__(self,arch = 'extiny' ,
        ...
        """"""
        self.backbone_channels_cat = [64 + 0, 128 + 64, 256 + 64*4, 512 + 64*16]

        self.conv1 = nn.Sequential(nn.Conv2d(64, 128, 1),nn.BatchNorm2d(128),nn.ReLU())
        self.conv2 = nn.Sequential(nn.Conv2d(256, 256, 1),nn.BatchNorm2d(256),nn.ReLU())
        self.conv3 = nn.Sequential(nn.Conv2d(64*16, 512, 1),nn.BatchNorm2d(512),nn.ReLU())

        self.deploy = deploy

        self.prenorm = nn.BatchNorm2d(1)
        ...
        self.sparse_head = nn.ModuleList([
            SparseConvNeXtBlock(
                in_channels = self.backbone_channels[-4]//4,
    ...
    def forward(self, input):
        origin = Denormalization(input, dict(mean=107.80905151367188, std=33.02274703979492)) / 255.0
        input = self.prenorm(input)
        ps8x = self.pixel_shuffle_down(self.pixel_shuffle_down(self.pixel_shuffle_down(origin)))
        ...
        feats4x = layers[0]
        # start_time = datetime.now()
        layers[-3] = layers[-3] + self.conv1(ps8x)
        layers[-2] = layers[-2] + self.conv2(ps16x)
        layers[-1] = layers[-1] + self.conv3(ps32x)
