...
from .mobiletwo256 import MobileTwo, MobileOneBlock
...
class ChannelReweight(nn.Module):
    ...
    def forward(self,x):
        ...
        return torch.bmm(x.reshape([B,C,-1]).permute(0,2,1),torch.softmax(torch.bmm(x_gap1,x_gap2),dim=-1)).permute(0,2,1).reshape([B,C,H,W])
...
class RepirDet(nn.Module):
    def __init__(self,arch = 'tiny' ,
        ...
        self.sparse_head = nn.ModuleList([
            ...
            MobileOneBlock(
                ...
                kernel_size=3,
                num_convs=4,
                groups=self.backbone_channels[-3],
                deploy=self.deploy),
            nn.Conv2d(
                self.backbone_channels[-3],
                self.backbone_channels[-3],
    ...
    def forward(self, input):
        ...
        #         outfeatures = {}
        #
        #         outfeatures["feats4x"] =F.normalize(feats4x,dim=0)
        #         outfeatures["feats8x"] = F.normalize(feats8x,dim=0)
        #         outfeatures["feats16x"] = F.normalize(feats16x,dim=0)
        #         visual(input, outfeatures)
        ...
        classify_results = F.interpolate(self.classify_head(feats8x), scale_factor=2, mode='bilinear')
        map = self.keep_topk(classify_results.sigmoid())
