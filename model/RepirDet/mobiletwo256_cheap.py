...
class MobileTwo(BaseBackbone):
    ...
    def __init__(self,
        ...
        super(MobileTwo, self).__init__(init_cfg)
        ...
        for i in range(num_blocks):
            ...
            if i == 0:
                ...
                blocks.append(
                    # Depthwise conv
                    MobileOneBlock(
                        in_channels=self.in_planes//2,
                        out_channels=self.in_planes//2,
                        kernel_size=3,
                        num_convs=num_conv_branches,
                        stride=strides[i],
                        padding=1,
                        groups=self.in_planes//2,
                        se_cfg=self.se_cfg if use_se else None,
                        conv_cfg=self.conv_cfg,
                        norm_cfg=self.norm_cfg,
                        act_cfg=self.act_cfg,
    ...
    def forward(self, x):
        ...
        for i, stage_name in enumerate(self.stages):
            ...
            x = stage(x)
            if len(outs) > 0:
                x = torch.concat([x, F.max_pool2d(outs[-1], kernel_size=3, stride=2, padding=1)], dim=1)
            elif len(outs) == 0:
                x = torch.concat([x, F.max_pool2d(feats2x, kernel_size=3, stride=2, padding=1)], dim=1)
    ...
    def train(self, mode=True):
        ...
        super(MobileTwo, self).train(mode)
        self._freeze_stages()
