...
class WTConv2d(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.wt_levels):
            ...
            # 对当前低频分量进行小波变换
            curr_x = self.wt_function(curr_x_ll)  # curr_x 是经过小波变换后的4通道的特征图
            # 提取小波变换后的低频分量
            curr_x_ll = curr_x[:, :, 0, :, :]
            ...
            # 在此处加入子带注意力机制
            # 计算子带之间的注意力权重
            subband_descriptors = curr_x_tag.mean(dim=[3, 4])  # [batch_size, channels, num_subbands] {1，16，4，32，32}
            attention_weights = F.softmax(subband_descriptors, dim=2)  # [batch_size, channels, num_subbands]
            # 将权重映射到每个子带上
            attention_weights = attention_weights.unsqueeze(-1).unsqueeze(
                -1)  # [batch_size, channels, num_subbands, 1, 1]
            curr_x_tag = curr_x_tag * attention_weights  # [batch_size, channels, num_subbands, height, width]
