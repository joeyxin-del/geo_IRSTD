...
# from .LKBackbone import CSPNeXt
...
from mmpretrain.models.backbones.cspnet import CSPResNet
# from mmpretrain.models.backbones.resnet import ResNet
# from .resnet import ResNet
# from .resnetv2 import ResNet
# from .resnetv3 import ResNet
from .resnetv4 import ResNet
# from mmpretrain import inference_model
import torch.nn.functional as F
from torch.nn.modules.utils import _triple, _pair, _single
import torchvision.utils as vutils

# from .EfficientVit import EfficientGatedViTBlock,efficientvit_bs,EfficientViTBlock,build_norm,build_act,get_same_padding,MBConv,IdentityLayer,ResidualBlock
...
class LinearVIT(nn.Module):
    def __init__(self, ):
        ...
        # self.backbone_channels = [64 // 2, 128 // 2, 256 // 2, 256 // 2] # resnet最后一层变成linearAttblock
        """"""
        self.backbone_channels_cat = [64 // 2 + 0, 128 // 2 + 64, 256 // 2 + 64 * 4, 512 // 2 + 64 * 16]
        # self.backbone_channels_cat = [64 // 2 + 0, 128 // 2 + 64, 256 // 2 + 64 * 4, 256 // 2 + 64 * 16]# resnet最后一层变成linearAttblock

        # self.backbone = CSPNeXt()
        # self.backbone = CSPResNet(in_channels=1, depth=25, out_indices=(0, 1, 2, 3))
        # self.backbone = ResNet(depth=10, in_channels=1, stem_channels=32, base_channels=32, out_indices=(0, 1, 2, 3))
        # self.backbone = efficientvit_bs(weights='checkpoints/b1.pt')
        self.backbone = ResNet(depth=14, in_channels=1, stem_channels=32, base_channels=32, out_indices=(0, 1, 2, 3))
        # self.backbone = efficientvit_bs()
    ...
    def forward(self, input):
        ...
        # print(input.shape)
        layers = list(self.backbone(input))  # CSPResNet
        # for i in layers:
        #     print(i.shape)
