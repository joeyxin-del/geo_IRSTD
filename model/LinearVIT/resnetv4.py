...
# from .wtconv.wtconv2dAssign import WTConv2d
# from .wtconv.wtconv2d import WTConv2d
eps = 1.0e-5


class linearAttBlock(BaseModule):
    ...
    def __init__(self,
                 ...
                 init_cfg=None):
        super(linearAttBlock, self).__init__(init_cfg=init_cfg)
        ...
        self.norm1_name, norm1 = build_norm_layer(
            norm_cfg, self.out_channels, postfix=1)
        ...
        self.conv2 = build_conv_layer(
            ...
            padding=1,
            bias=False)
        # self.conv2 = WTConv2d(in_channels = self.out_channels, out_channels = self.out_channels, kernel_size=3)
        ...
        self.linear_att = EfficientViTBlock(
    ...
    def forward(self, x):
        ...
        if self.with_cp and x.requires_grad:
            ...
            if self.downsample is not None:
                ...
                out = self.linear_att(x)
        return out

class NoiseBlock(BaseModule):
    """BasicBlock for ResNet with Linear Noise.
    ...
    Args:
        ...
        norm_cfg (dict): dictionary to construct and config norm layer.
            Default: dict(type='BN')
        noise_strength (float, optional): Strength of the linear noise to be added. Default: 0.0
    """
    ...
    def __init__(self,
                 ...
                 init_cfg=None,
                 noise_strength=0.5):
        super(NoiseBlock, self).__init__(init_cfg=init_cfg)
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.expansion = expansion
        assert self.expansion == 1
    ...
    def forward(self, x):
        ...
        def _inner_forward(x):
            ...
            # Add linear noise before conv2
            if self.noise_strength > 0.0:
                x_copy = out.detach()  # Copy x and detach gradient
                x_copy = torch.cat((x_copy[1:, :, :, :], x_copy[0, :, :, :].unsqueeze(0)), dim=0)  # Circular shift
                noise = (x_copy - out.detach())  # Generate linear noise
...
class ResNet(BaseBackbone):
    ...
    arch_settings = {
        8: ((BasicBlock,BasicBlock,BasicBlock,BasicBlock), (2, 2, 2, 2)),
        10: ((BasicBlock, BasicBlock, BasicBlock, NoiseBlock), (2, 2, 2, 2)),
        14: ((BasicBlock, BasicBlock, BasicBlock, linearAttBlock), (2, 2, 2, 2)),
    ...
    def __init__(self,
        ...
        self.block, stage_blocks = self.arch_settings[depth]    # 18: (BasicBlock, (2, 2, 2, 2)),
        self.stage_blocks = stage_blocks[:num_stages]   # (2, 2, 2, 2)
        self.expansion = get_expansion(self.block[0], expansion)
...
if __name__ == '__main__':
    net = ResNet(depth=14,in_channels=1,stem_channels=32,base_channels=32,out_indices=(0,1,2,3))
    torchinfo.summary(net)
    out = list(net(torch.randn(1,1,512,512)))
