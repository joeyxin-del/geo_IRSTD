...
def get_optimizer(net, optimizer_name, scheduler_name, optimizer_settings, scheduler_settings):
    if optimizer_name == 'Adam':
        ...
        optimizer = torch.optim.AdamW(net.parameters(), lr=optimizer_settings['lr'], weight_decay=0.05)
    elif optimizer_name == 'NAdam':
        optimizer = torch.optim.NAdam(net.parameters(), lr=optimizer_settings['lr'], weight_decay=1e-4)
    elif optimizer_name == 'Lion':
        optimizer = Lion(net.parameters(), lr=optimizer_settings['lr'])
    if scheduler_name == 'MultiStepLR':
        ...
        scheduler  = torch.optim.lr_scheduler.CyclicLR(optimizer,0.00005,0.0002,500,500,cycle_momentum=True)
